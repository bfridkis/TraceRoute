...
$HostsForTraceRT = New-Object System.Collections.Generic.List[System.Object]
...
if ($readFileOrManualEntry -eq 1) {
    ...
    $HostsForTraceRT = Get-Content $hostsFilePath -ErrorAction Stop
}
...
#Process lookup...
$HostsForTraceRT | ForEach-Object {
    $thisHost = $_
    Try { 
        ...
        $thisResult.TraceRoute | ForEach-Object {
            $results.Add([PSCustomObject]@{'Hostname/IP Address'=$thisHost;
                                            'Hop Number'=$count++;
                                            'Hop Address'= "$_";
                                            'Hop Hostname' = Try { $([system.net.dns]::GetHostByAddress($_)).Hostname } Catch { "No Hostname Available" }})
        }
    }
